...
public:
	...
	void MakeShader(const std::string name, bool useShaderSwizzle = false);
	Shader(const char *name);
	Shader(const std::string name, bool useShaderSwizzle = false);

	GLuint Object() const noexcept;
	GLint Attrib(const char *name) const;
	GLint Uniform(const char *name) const;

	static std::string ShaderPath(std::string shader, bool isFragment, bool useShaderSwizzle = false);
